...
function busy:check()
	 wait.make(function()
	  run(checkcmd)
      local l,w=wait.regexp('^.*(自杀有两种，您是要永远死掉\\w*|设定环境变量：no_more = "busy"$)',self.timeout) --超时
      if l==nil then
		print "网络太慢或是发生了一个非预期的错误"
		self:check()
		return
	  end
	  if string.find(l,"自杀有两种，您是要永远死掉") then
		 self:finish()
	     return
	  end
...
system.update=function()
	...
	addtri("system_nobusy","^.*(自杀有两种，您是要永远死掉\\w*|你现在不忙\\w*|你不忙\\w*|你不忙$)","system","")
